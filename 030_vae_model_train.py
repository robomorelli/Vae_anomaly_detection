...
def train_vae(name, w, intermediate_dim, act_fun, latent_dim, kernel_max_norm,
        ...
        train_history = vae.fit(x=train, y=[train, train],
            ...
            callbacks =
                        [TerminateOnNaN(),
                        ModelCheckpoint(model_results_single + '{}/vae_{}.h5'.format(name, name),
                                            monitor='val_loss',
                            ...
                            ReduceLROnPlateau(monitor='val_loss',
                                              ...
                                              min_lr=9e-8,
                                              verbose=1),
                        SaveAutoencoder(model_results_single +'{}/autoencoder_{}.h5'.format(name, name),
                                            monitor='val_loss',
                                            mode='auto', save_best_only=True,verbose=1,
                                            period=1)
                                            ])
        ...
        # or save to csv:
        hist_csv_file = model_results_single +'{}/history_{}.csv'.format(name, name)
...
if __name__ == "__main__":
    ...
    try:
        os.makedirs(model_results_single + name)
    except:
        pass
    ...
    train_vae(name, w=2, intermediate_dim=50, act_fun='relu',latent_dim=4, kernel_max_norm=500,
                lr=0.003,epochs=2000, weight_KL_loss=0.6, batch_size=200)
